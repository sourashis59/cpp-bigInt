...
private:
    friend int toInt(char c);
    friend char toChar(int n);
    friend void removeZerosFromFrontOfString(string &s);
    friend string addStrings(string s1, string s2);
    friend string multiplyPositiveStrings(string s1, string s2);
...
public:
    ...
    bool isEven() const;
    bool isOdd() const;
    friend bigInt pow(const bigInt &a, const bigInt &n); // ONLY SUPPORTS POSITIVE VALUE FOR n
